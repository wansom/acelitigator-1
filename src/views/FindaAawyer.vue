  ...
  <div>
    ...
    <main id="app" class="content">
      <nav class="nav">
        ...
        <label class="nav__label" @click="modal = !modal">Create Account</label>
      </nav>
      ...
      <transition-group name="company" tag="ul" class="content__list">
        <li class="company" v-for="advocate in list" :key="advocate.id">
          <div class="company__info">
            <icon
              class="company__logo"
              
              :use="advocate.profile_photo"
            ></icon>
            <h2 class="company__name">{{ advocate.first_name }}</h2>
          ...
          <ul class="company__details">
            <li class="company__data">
              <label class="company__label">County</label>
              <p
                class="company__county"
                @click="clearFilter('counties', advocate.county)"
              >
                {{ advocate.location }}
              </p>
            </li>
            ...
            <li class="company__data">
              <label class="company__label">Experience</label>
              <p class="company__experience">{{ advocate.email }}</p>
            </li>
          </ul>
        </li>
      </transition-group>
...
export default {
  ...
  return {
    ...
    filters: { counties: {}, practiseAreas: {}, experience: 0 },
    menus: { counties: false, practiseAreas: false, experience: false }
  };
},
computed: {
  ...mapState([
  ...
  list() {
    let { counties, practiseAreas } = this.activeFilters

    return this.allAdvocates.filter(({ location, practise_areas, experience }) => {
      if (experience < this.filters.experience) return false
      if (counties.length && !~counties.indexOf(location)) return false
      return !practiseAreas.length || practiseAreas.every(cat => ~practise_areas.indexOf(cat))
    })
  },
  activeFilters() {
    let { counties, practiseAreas } = this.filters

    return {
      counties: Object.keys(counties).filter(c => counties[c]),
      practiseAreas: Object.keys(practiseAreas).filter(c => practiseAreas[c]),
      experience: (this.filters.experience > this.experience.min) ? [this.filters.experience] : []
    }
  }
},
methods: {
...
beforeMount() {
  ...
  this.allAdvocates.forEach(({experience }) => {
    // this.$set(this.filters.counties, this.counties, false)
