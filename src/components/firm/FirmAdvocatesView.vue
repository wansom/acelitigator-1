<template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4" >
      <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700" v-for="(member,index) of user.teamMembers" :key="index">

<div class="flex flex-col items-center pb-10">
    <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src="/images/profiles/avatar.png" alt="Bonnie image"/>
    <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{{ member.name }}</h5>
    <span class="text-sm text-gray-500 dark:text-gray-400">{{ member.job_title }}</span>
...
import { updateAdvocate, updateTeamMember } from "../../database/firestore";
    ...
    async handleSubmit(e) {
      ...
      this.form.validateFields(async (err, values) => {
        if (!err) {
          ...
          updateTeamMember(this.user.id,payload,'add')
        }
      });
    },
  }
